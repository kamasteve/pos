...
$balance=$_POST['available'];
if($balance<-1){
$res ="SELECT code as number FROM invoice";
		...
		$user=$_SESSION['user_name'];
		$i ="update invoice set code='$number'";
...
$ip=get_client_ip() ;
		...
		$cash=$_POST['cash'];
		$change=$_POST['change'];
		$D="SELECT * FROM sales where number='$receipt' and type='INVOICE'";
...
VALUES ('$name', '$qty', ('$TOTAL'), '$receipt', '$date', NOW(), '$price', '$code','$buying',round('$totalv'),'$user','$x','$system','$current','INVOICE')";
...
$ff=mysqli_query($dbc,$f);
	...
	$sl=GetSALES();
$ch=$_SESSION['customer'];
	$cc=GetCostG();
$sss=GetStock();
	SalesRecINC($totall,$receipt,$sl,$ch,$session,$_SESSION['expected'],$_SESSION['date']);
	...
	$date=$_SESSION['date'];
		...
		values(NOW(),'$tiller','$user','$totall','$date','INVOICE','$receipt','$x')";
		$sql=mysqli_query($dbc,$t);
		UserAudit($_SESSION['user_id'],"Added Product Sales Invoiceo No $receipt",date("Y/m/d"),$ip);
	$url="invoice.php?c=$cash&r=$receipt&ch=$change&x=$x&customer=$ch";
	ob_end_clean();
header("Location: $url");
		...
		unset($_SESSION['type']);
		unset($_SESSION['balanced']);
	unset($_SESSION['RECEIPT']);
}
} //if($balance<0){
//$_SESSION['msg']='Transaction Not Possible Credit Limit Breached';
...
<html>
<script language="JavaScript" type="text/javascript" src="creditor.js"></script>
...
//krsort($_SESSION["products"]);
  ...
  $i = 0;
	...
	echo '<form method="post" action="add_pp.php">';
	?>
  ...
  <tr><td  style="border-color:#000000;background-color:#e8e8e8;" width="500"><?echo UCWORDS($cart_itm["name"]);?></td>
	...
	<?echo
	  ...
	  <td style="border-color:#000000;background-color:#e8e8e8;" width="5"><?echo '<a href=creditors.php?removep=' . $cart_itm["product_id"] .'>';?>
	<img src="x.png"/></a></td>
...
name="type" value="Update" style="height: 0px;background-color#4AC948;  width: 0px;
  ...
  <form name="cart" id='inputform'  method="POST" action="creditors.php" style="border-color:#000000; border-width:2; font-size:17px;">
 <tr>
   ...
   <tr>
      <td colspan="3" align="right" style="border-color:#000000;background-color:#e8e8e8;">Credit Limit</td>
      <td style="background-color:#e8e8e8;" width="130"><input type="text"  name="limit" value="<?echo $_SESSION['limit'];?>" style="border-color:#000000;background-color:#bbbbbb"; id="limit" readonly/></td>
  </th></tr>
    <tr>
      <td colspan="3" align="right" style="border-color:#000000;background-color:#e8e8e8;">Outstanding Balance</td>
      <td style="background-color:#e8e8e8;" width="130"><input type="text"  name="outstanding" value="<?echo number_format($_SESSION['balanced']);?>" style="border-color:#000000;background-color:#bbbbbb"; id="outstanding" readonly/></td>
  </th></tr>
   <tr>
      <td colspan="3" align="right" style="border-color:#000000;background-color:#e8e8e8;">Available Balance</td>
      <td style="background-color:#e8e8e8;" width="130"><input type="text"  name="available" value="<?echo $_SESSION['limit']-($totalp+$_SESSION['balanced']);?>" style="border-color:#000000;background-color:#bbbbbb"; id="available" readonly/></td>
  </th></tr>
