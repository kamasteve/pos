...
include('mysql.php');
$credit=$_SESSION['OUT'];
$receipt=$_SESSION['SESS'];

$s="select * from stocking where t_code='$receipt'";
$sql=mysqli_query($dbc,$s);

$sq=mysqli_fetch_array($sql);
$supplier=Mode($sq['supplier']);

 DebitNote($credit,$receipt,$supplier);
unset($_SESSION['OUT']);
	unset($_SESSION['SESS']);	
	$r="update ccode set code=code+1";
