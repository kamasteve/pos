...
$csv_hdr="Cumulative Sales Report  For Dates Between $date1 And $date2 \n";
    $csv_hdr .= "Product, Qty, Average Selling Price,Sub Total";
...
// Lets say we wanted a table with all orders, their products and totals...a summary report of sorts
    $result=mysql_query("SELECT * ,sum(qty)as qty,sum(total_p)as total from sales where 
...
//here we are displaying the contents of the field or column in our rows array for a particular row.
          ...
          //repeat for all remaining fields or columns we have headings for...
            $csv_output .= $row['qty'] . ", ";
           $csv_output .= $row['total']/$row['qty'] . ", ";
            $csv_output .= $row['total'] . ", ";
