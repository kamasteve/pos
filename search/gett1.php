...
function namesearch($m) {
  ...
  $name=$row['name'];
    if ($name=='MPESA') {
			...
			echo' <div class="art-content-layout-row"><form method="POST" action="tables1.php" autocomplete="off">
                                                                            <label> Code</label>
   ...
   echo '   <button name="add">Add  Transaction</button>
	    ...
	    if (($name)!='MPESA'&& $name!='CASH') {
			...
			echo' <form method="POST" action="tables1.php" autocomplete="off">
                                                                            <label>Amount</label>
	...
	echo'
	  ...
	  if ($name=='CASH') {
			...
			echo'
                                                             <form method="POST" action="tables1.php" autocomplete="off"><tr>
                                                                       <label>Amount</label>
                                                                           <input    name="amountt" autofocus value="" placeholder="Enter Amount."
																			title="Enter Amount." required  />';	
	echo'
