...
<table class="tables" border="1" cellspacing="0">
  ...
  echo '<tr><td><a href="change_g.php?id=' . ($row['gnumber']) . ' &t=' . ($row['transid']) . '&h=' . ($row['held']) . '">Transfer Guarantee</a></div></td><td>' .($row['membernumber']). '</td><td>' .($row['shares']). '</td><td>' .LoanName($row['transid']). '</td><td>' .number_format($row['amount'],0). '</td>
