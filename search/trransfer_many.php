...
<table class="tables" border="1" cellspacing="0">
        <tr><th>Transfer</th><th>Member</th><th>Shares</th><th>Loan Type</th><th>Guaranteed</th><th>Freed</th><th>Held</th></tr>';
  ...
  echo '<tr><td><a href="change_many.php?id=' . ($row['gnumber']) . ' &t=' . ($row['transid']) . '&h=' . ($row['held']) . '">Transfer Guarantee</a></div></td><td>' .($row['membernumber']). '</td><td>' .($row['shares']). '</td><td>' .LoanName($row['transid']). '</td><td>' .number_format($row['amount'],0). '</td>
  <td>' . number_format(($row['rem']),0). '</td><td>' . number_format(($row['held']),0). '</td></tr>';
  }
    echo '<tr><td colspan="4"><b>TOTAL</td><td><b>' .number_format($g,0). '</td>
