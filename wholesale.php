...
foreach ($_SESSION["wholesale"] as $cart_itm)
    {
...
multiple($credit1,$cheque,$to,$rec,$gtotal);
  ...
  unset($_SESSION['SESSION']);
	...
	if(isset($_POST["submit"])&&isset($_SESSION["wholesale"])){
...
header("Location: $url");
	...
	unset($_SESSION['RECEIPT']);
			...
			if(isset($_GET["removep"]) && isset($_SESSION["wholesale"]))
{
	$product_id	= $_GET["removep"]; //get the product code to remove
	//$return_url 	= base64_decode($_GET["return_url"]); //get return url

	
	foreach ($_SESSION["wholesale"] as $cart_itm) //loop through session array var
...
<html>
<script language="JavaScript" type="text/javascript" src="wholesale.js"></script>
...
<?if(isset($_SESSION["wholesale"]))
{
//krsort($_SESSION["wholesale"]);
    $total = 0;
  $i = 0;
    foreach ($_SESSION["wholesale"] as $cart_itm)
    {
	$totalp+=$cart_itm["price"]*$cart_itm["qty"];
	echo '<form method="post" action="add_w.php">';
  ...
  <tr><td  style="border-color:#000000;background-color:#e8e8e8;" width="500"><?echo UCWORDS($cart_itm["name"]);?></td>
	...
	<?echo
	  ...
	  <td style="border-color:#000000;background-color:#e8e8e8;" width="5"><?echo '<a href=wholesale.php?removep=' . $cart_itm["product_id"] .'>';?>
	<img src="x.png"/></a></td>
...
name="type" value="Update" style="height: 0px;background-color#4AC948;  width: 0px;
  ...
  <form name="cart" id='inputform'  method="POST" action="wholesale.php" style="border-color:#000000; border-width:2; font-size:17px;">
  <tr>
